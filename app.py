...
if 'hse_questions' not in st.session_state:
    st.session_state.hse_questions = {
        "Demandas": [
            ...
            {"id": 16, "q": "Não consigo fazer pausas suficientes?", "rev": True, "help": "Ex: Sem horário de almoço."},
            {"id": 18, "q": "Sou pressionado por diferentes grupos?", "rev": True, "help": "Ex: Conflito de ordens."},
            {"id": 20, "q": "Tenho que trabalhar muito rápido?", "rev": True, "help": "Ex: Ritmo frenético."},
            {"id": 22, "q": "Tenho prazos irrealistas?", "rev": True, "help": "Ex: Metas inatingíveis."}
        ],
        "Controle": [
...
def admin_dashboard():
    ...
    if selected == "Visão Geral":
        ...
        with tab2:
            ...
            with st.form("add_comp"):
                ...
                if st.form_submit_button("Salvar"):
                    new_c = {"id": cod, "razao": razao, "cnpj": cnpj, "setor": "Geral", "risco": risco, "func": func, "segmentacao": "GHE", "resp": "RH", "email": "-", "logo": None, "score": 0, "respondidas": 0, "dimensoes": {}}
        ...
        with st.expander("📝 Editar Conteúdo Técnico", expanded=True):
            analise_texto = st.text_area("Análise Executiva (Conclusão):", value="A avaliação identificou riscos críticos na dimensão 'Demandas', com 65% dos colaboradores relatando prazos irreais.")
            ...
            edited_df = st.data_editor(pd.DataFrame(st.session_state.acoes_list), num_rows="dynamic", use_container_width=True, 
                                       column_config={
                                           "acao": "Ação Macro",
                                           "detalhes": "Como Desenvolver (Detalhes)",
                                           "area": "Área HSE",
                                           "resp": "Responsável",
                                           "prazo": "Prazo"
                                       })
        ...
        if st.button("🖨️ Gerar Relatório Moderno (PDF)", type="primary"):
            ...
            # Geração das 35 Perguntas com Barras de Progresso
            html_detalhe_geral = ""
            for categoria, perguntas in st.session_state.hse_questions.items():
                html_detalhe_geral += f'<div style="margin-top:15px; margin-bottom:5px; font-weight:bold; color:{COR_PRIMARIA}; border-bottom:1px solid #eee;">{categoria}</div>'
                for q in perguntas:
                    # Simulação de % de risco para o relatório (já que não temos banco real cheio)
                    pct_risco = random.randint(10, 80) 
                    cor_bar = COR_RISCO_ALTO if pct_risco > 50 else (COR_RISCO_MEDIO if pct_risco > 25 else COR_RISCO_BAIXO)
                    html_detalhe_geral += f"""
                    <div style="margin-bottom:6px; font-size:10px;">
                        <div style="display:flex; justify-content:space-between;">
                            <span style="color:#555;">{q['q']}</span>
                            <span style="font-weight:bold; color:{cor_bar}">{pct_risco}% Risco</span>
                        </div>
                        <div style="width:100%; background-color:#f0f0f0; border-radius:3px; height:6px;">
                            <div style="height:100%; border-radius:3px; width:{pct_risco}%; background-color:{cor_bar};"></div>
                        </div>
                    </div>
                    """

            # HTML Cards Dimensões
            html_dimensoes = ""
...
<div>{logo_html}</div>
<div style="text-align:right;"><div style="font-size:18px; font-weight:700; color:{COR_PRIMARIA};">LAUDO TÉCNICO HSE-IT</div><div style="font-size:11px; color:#666;">NR-01 / Riscos Psicossociais</div></div>
</div>
...
<div style="font-size:10px;">CNPJ: {empresa['cnpj']} | Adesão: {empresa['respondidas']} Vidas | Emissão: {datetime.datetime.now().strftime('%d/%m/%Y')}</div>
</div>
...
<p style="text-align:justify; margin:0;">Este laudo tem como objetivo identificar fatores de risco psicossocial conforme a NR-01. A metodologia utilizada foi a <strong>HSE Indicator Tool</strong>, que avalia 7 dimensões do trabalho através de 35 indicadores-chave, garantindo anonimato e análise estatística dos grupos.</p>
</div>
...
<div style="background:white; border:1px solid #eee; padding:10px; border-radius:6px; column-count: 2; column-gap: 20px;">{html_detalhe_geral}</div>
